...
func CreateCommandStreamAbandon() *cobra.Command {
	stream_abandon := &cobra.Command{
		...
		Run:   HandleCommandStreamAbandon,
	}

	stream_abandon.Flags().String("claim_id", "", "(str) claim_id of the claim to abandon")
	stream_abandon.Flags().String("txid", "", "(str) txid of the claim to abandon")
	stream_abandon.Flags().Int("nout", -1, "(int) nout of the claim to abandon")
	stream_abandon.Flags().String("account_id", "", "(str) id of the account to use")
	stream_abandon.Flags().String("wallet_id", "", "(str) restrict operation to specific wallet")
	stream_abandon.Flags().Bool("preview", false, "(bool) do not broadcast the transaction")
	stream_abandon.Flags().Bool("blocking", false, "(bool) wait until abandon is in mempool")

	return stream_abandon
}

func HandleCommandStreamAbandon(cmd *cobra.Command, args []string) {

...
func CreateCommandChannelAbandon() *cobra.Command {
	channel_abandon := &cobra.Command{
		...
		Run:   HandleCommandChannelAbandon,
	}

	channel_abandon.Flags().String("claim_id", "", "(str) claim_id of the claim to abandon")
	channel_abandon.Flags().String("txid", "", "(str) txid of the claim to abandon")
	channel_abandon.Flags().Int("nout", -1, "(int) nout of the claim to abandon")
	channel_abandon.Flags().String("account_id", "", "(str) id of the account to use")
	channel_abandon.Flags().String("wallet_id", "", "(str) restrict operation to specific wallet")
	channel_abandon.Flags().Bool("preview", false, "(bool) do not broadcast the transaction")
	channel_abandon.Flags().Bool("blocking", false, "(bool) wait until abandon is in mempool")

	return channel_abandon
}

func HandleCommandChannelAbandon(cmd *cobra.Command, args []string) {
